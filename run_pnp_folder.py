...
def main():
    ...
    parser.add_argument('--config', default='configs/pnp/pnp-sunnyday2snowynight-v2.yaml')
    parser.add_argument("--output_folder", type=str, default='drivingedition_results/sunnyday2snowynight-v2', help="output folder for results")
    parser.add_argument("--jsonl_file", type=str, default="/home/turing/cfs_cz/finn/codes/DrivingEdition/examples/text_to_image/data/jidu-data-III-sensors/jidu-III-sunnyday2snowynight-v2/metadata.jsonl", help="jsonl file containing prompts")
    ...
    ddim_steps = exp_config.num_ddim_sampling_steps if exp_config.num_ddim_sampling_steps is not None else \
    possible_ddim_steps[-1]
    ...
    # load jsonl files
    prompt_dict={}
    with open(opt.jsonl_file, 'r') as file:
        for line in file:
            data = json.loads(line)
            txt = data['text']
            if exp_config.source_weather in txt:
                txt = txt.replace(exp_config.source_weather, exp_config.target_weather)
                prompt_dict[data['file_name'].split('.')[0]] = txt
    folder_list = os.listdir(os.path.join(exp_path_root, exp_config.source_experiment_name))
    folder_list = [f for f in folder_list if not f.endswith('.json')]
    for folder in folder_list:
        def load_target_features():
            ...
            source_experiment_out_layers_path = os.path.join(exp_path_root, exp_config.source_experiment_name,folder,
                                                             "feature_maps")
            source_experiment_qkv_path = os.path.join(exp_path_root, exp_config.source_experiment_name, folder,"feature_maps")
        ...
        batch_size = 1
        prompts = prompt_dict[folder]
        assert prompts is not None

        start_code_path = f"{exp_path_root}/{exp_config.source_experiment_name}/{folder}/z_enc.pt"
            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for k, x_sample in enumerate(x_image_torch):
                        ...
                        img.save(os.path.join(opt.output_folder, f"{folder}.jpeg"))
                        sample_idx += 1
